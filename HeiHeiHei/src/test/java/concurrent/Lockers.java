 ...
 *      则立即返回,将保持计数器加1;如果锁定被其他线程占据,则当前线程进入睡眠状态,等待其他线程释放锁,此时保持计数器置为1.
 * 3.ReentrantLock的unlock方法释放锁,如果当前线程是锁的占有者,则将保持计数器减1,如果保持计数器等于0,则释放锁.如果当前线程不是锁的占有者,
 *      则抛出异常.
 ...
 * （这跟事务的 serializable 数据库隔离级别很像， 如果有写入 禁止一切读，读的时候 禁止一切写入）
 */
public class Lockers{
	...
	public static void testLockTest() throws Exception{
		...
		future.get();//等待任务2执行完后,关闭 任务执行服务
